...
export class Editor2 implements Editor {
  ...
  handleCanvasClick(event: PointerEvent) {
    ...
    if (selectedIndex !== -1) {
      ...
      if (this.selectedContours.has(selectedIndex)) {
        ...
        this.reDrawContour(selectedIndex, Colors.RED, false);
      }
    }
  }
  ...
  private reDrawAllContours(colour: any, doNotify = true): void {
    this.reDrawContours([-1], colour, doNotify);
  }
  private reDrawContour(ind: number, colour: any, doNotify = true): void {
    this.reDrawContours([ind], colour, doNotify);
  }
  private reDrawContours(inds: number[], colour: any, doNotify = true): void {
